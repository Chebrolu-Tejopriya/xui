...
// ---- Codex's MCP config ------------------------------------------------------
//
// Every XUI tool is pre-approved, and the list is read from the server itself.
// The tools only READ the generated contract, so there is nothing to approve.
// Without this, clicking "always allow" in Codex writes the approval into THIS
// file - which is committed - and leaves everyone's git status dirty with a
// change nobody meant to make. Found in the first real Codex test.
const server = fs.readFileSync(path.join(root, 'mcp/server.mjs'), 'utf8');
const tools = [...server.matchAll(/^ {4}name: '([a-z_]+)',$/gm)].map((m) => m[1]);
if (tools.length === 0) {
  throw new Error('found no tools in mcp/server.mjs - has the `    name: \'...\',` shape changed?');
}
fs.writeFileSync(
  path.join(root, '.codex/config.toml'),
  [
    '# GENERATED by scripts/gen-agent-rules.mjs from mcp/server.mjs. Do not edit -',
    '# change the server and run npm run ds:build.',
    '#',
    "# XUI's MCP server, for Codex. Codex reads a project's .codex/config.toml only",
    '# once you trust the project. Same server as .mcp.json (Claude Code) and',
    '# .cursor/mcp.json (Cursor): zero dependencies, reads the generated contract.',
    '[mcp_servers.xui]',
    'command = "node"',
    'args = ["mcp/server.mjs"]',
    '',
    '# Pre-approved: every tool only reads. Otherwise "always allow" writes this',
    '# file, and it is committed.',
    ...tools.flatMap((t) => ['', `[mcp_servers.xui.tools.${t}]`, 'approval_mode = "approve"']),
    '',
  ].join('\n'),
);
...
console.log(
  `agent rules: ${skills.length} skills -> .cursor/rules/xui-*.mdc and .agents/skills/ ` +
    ...
    `${tools.length} MCP tools pre-approved -> .codex/config.toml`,
);
